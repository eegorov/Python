...
validate_node(node *tree)
{
    ...
    type -= NT_OFFSET;
    if (type >= get_PyParserGrammar()->g_ndfas) {
        PyErr_Format(parser_error, "Unrecognized node type %d.", TYPE(tree));
        return 0;
    }
    const dfa *nt_dfa = &get_PyParserGrammar()->g_dfa[type];
    ...
    for (pos = 0; pos < nch; ++pos) {
        ...
        int ch_type = TYPE(ch);
        if ((ch_type >= NT_OFFSET + get_PyParserGrammar()->g_ndfas)
        ...
        for (arc = 0; arc < dfa_state->s_narcs; ++arc) {
            ...
            assert(a_label < get_PyParserGrammar()->g_ll.ll_nlabels);

            const char *label_str = get_PyParserGrammar()->g_ll.ll_label[a_label].lb_str;
            if ((get_PyParserGrammar()->g_ll.ll_label[a_label].lb_type == ch_type)
        ...
        /* What would this state have accepted? */
        {
            ...
            int next_type = get_PyParserGrammar()->g_ll.ll_label[a_label].lb_type;
            const char *expected_str = get_PyParserGrammar()->g_ll.ll_label[a_label].lb_str;

            if (ISNONTERMINAL(next_type)) {
                PyErr_Format(parser_error, "Expected %s, got %s.",
                             get_PyParserGrammar()->g_dfa[next_type - NT_OFFSET].d_name,
                             ISTERMINAL(ch_type) ? _PyParser_TokenNames[ch_type] :
                             get_PyParserGrammar()->g_dfa[ch_type - NT_OFFSET].d_name);
            }
